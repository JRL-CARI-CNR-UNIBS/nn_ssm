...
# Get paths
PATH = os.path.dirname(os.path.abspath(__file__)) + "/data/dq_as_input/"
nn_path_shared = PATH + nn_name

for d in range(len(list_dataset_name)):
  dataset_name = "ssm_dataset_dq_"+list_dataset_name[d]+".bin"
  ...
  # Input: parent, child, dq, (x,y,z) of obstacle,
  # Output: length, poi_first, poi_mid, poi_last, v_safe first, v_safe mid, v_safe last, speed first, speed mid, speed last,
  # dist first, dist mid, dist last, scaling first, scaling mid, scaling last, scaling
  n_input = dof+dof+dof+3
  n_output = 1+3*3+13
          ...
          with torch.no_grad():  # evaluation does not need gradients computation
              ...
              for i, batch in enumerate(val_dataloader, 0):
                ...
                #   loss = criterion(predictions**3, batch_targets**3)
                  ...
                  val_length.extend(predictions[:,-23].detach().cpu().numpy())
                  val_target_length.extend(batch_targets[:,-23].detach().cpu().numpy())

                  val_poi_x.extend(predictions[:,-19].detach().cpu().numpy())
                  val_target_poi_x.extend(batch_targets[:,-19].detach().cpu().numpy())

                  val_poi_y.extend(predictions[:,-18].detach().cpu().numpy())
                  val_target_poi_y.extend(batch_targets[:,-18].detach().cpu().numpy())

                  val_poi_z.extend(predictions[:,-17].detach().cpu().numpy())
                  val_target_poi_z.extend(batch_targets[:,-17].detach().cpu().numpy())
